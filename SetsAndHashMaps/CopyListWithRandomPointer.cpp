...
//other methods are in LinkedList/DoulblyLinkedList/CopyListWithRandomPointer.cpp file
Node* copyRandomList(Node* head) {
    ...
    temp=head;
    b=a->next;
    unordered_map<Node*,Node*>m;// <ogNode,duplicateNode>
    while(temp!=NULL){
        m[temp]=b;
        temp=temp->next;
        b=b->next;
    }
    for(auto x:m){//mapping random for duplicates
